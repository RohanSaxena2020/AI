...
base_path = "/Users/rohansaxena/Desktop/AI Engineer/Week 23-24 - Machine Learning Projects with Deployment/Classification Project - Non Neural Network/server/"

def classify_image(image_base64_data, file_path=None):
    _model = load_saved_artifacts()  # Load the model
...
def load_saved_artifacts():
    ...
    global _model  # Fix variable naming

    with open(base_path + "artifacts/class_dictionary.json", "r") as f:
        __class_name_to_number = json.load(f)
        __class_number_to_name = {v: k for k, v in __class_name_to_number.items()}

    if _model is None:  # Check if _model is None
        with open(base_path + 'artifacts/saved_model.pkl', 'rb') as f:
            _model = joblib.load(f)
    print("loading saved artifacts...done")
    return _model  # Return the loaded model


def get_cv2_image_from_base64_string(image_file):
    image_stream = np.asarray(bytearray(image_file), dtype=np.uint8)
